		...
		for(int y = 0; y < temp.length*40; y+=40)
		{
			for(int x = 0; x < temp[i].length*40; x+=40)
			{
				...
				if(temp[i][j] == 'X')
					...
					g.drawImage(wall,x, y, x+40, y+40,0, 0, wall.getWidth(), wall.getHeight(), null);
					if(board.dragonsAllDead())
						g.drawImage(door.get(1),x, y, x+40, y+40,0, 0, door.get(1).getWidth(), door.get(1).getHeight(), null);
					else
						g.drawImage(door.get(0),x, y, x+40, y+40,0, 0, door.get(0).getWidth(),door.get(0).getHeight(), null);
				}
